...
> **⚡️ 快速部署專案推薦：**
> 如果你或你的 AI 助手（Agent）需要快速部署本機端 API，建議直接參考並使用我們整理好的獨立專案倉庫：
> 👉 **[vetscribe-local-api (GitHub Repo)](https://github.com/blackghost1989/vetscribe-local-api)**
> （可直接將此連結提供給 Agent 讀取，以獲取最完整的環境設定與程式碼）
...
如果你希望完全不透過網路傳輸任何文字給 Gemini 或 OpenAI，可以使用 **Ollama** 或其他本機 LLM 工具來執行 Qwen 文字模型進行分析：

1. **安裝 Ollama**: 前往 [ollama.com](https://ollama.com/) 並下載安裝。
2. **執行 Qwen 模型**: 開啟終端機輸入 `ollama run qwen2.5:7b` (或其他你想要的型號，如 1.7b, 14b, 32b)。
3. **在網頁端設定**:
   - 點擊 **⚙️ 設定**。
   - 在本機設定區塊，**Local LLM API URL** 填入 `http://localhost:11434/v1` (Ollama 預設)。
   - **分析與分類模型** 選擇 `Local Qwen (LLM)`。
   - 點擊儲存。

如此一來，從語音轉錄到最後的重點分類，整個過程都只在你本機的 GPU/CPU 上運行，達到 100% 的隱私保護。
